...
- 2. 要求
  - 一个 promise 必须有 3 个状态，pending、fulfilled(resolved)、rejected。当处于 pending 状态的时候，可以转移到 fulfilled(resolved) 或者 rejected 状态，当处于 fulfilled(resolved) 或者 rejected 状态时，不可变。
